...
class NtripClient extends EventEmitter {
  ...
  constructor(options) {
    ...
    /**
     * @member {boolean} useTls - whether to use TLS/SSL for the connection
     */
    this.useTls = options.useTls || false;
    /**
     * @member {object} tlsOptions - options passed to tls.connect, e.g., { rejectUnauthorized, ca, cert, key }
     */
  ...
  _onData(data) {
    if (!this.isReady) {
      ...
      if (response.startsWith(config.REPLY_ACCEPT) || response.startsWith(config.REPLY_LEGACY_ACCEPT)) {
        this.isReady = true;
      }
    }
    this.emit('data', data);
